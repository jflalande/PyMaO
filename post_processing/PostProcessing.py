...
Strings must be enclosed in <""> in the JSON file. For example:

$..Unzip.status == \"done\"

This expression compares if the string (or variable) '$..Unzip.status' is equal to the string 'done'.
For our purposes, all the JSONPath expressions are variables.

TODO:
- Change config JSON to accept "histogram" with JSONPath expression
